...
class DriftNet(object):
    def __init__(self, config):
        ...
        self.drift_term = torch.tensor(pd.read_csv(self.data_config.drift_file, header=None).values, dtype=torch.float32).to(self.device) 

        # Dimensions of the data
        self.input_dim = self.state.size(1) # input dimension
        self.output_dim = self.drift_term.size(1) # output dimension 
        
        # Model 
        self.model = FeedForwardNet(config, self.input_dim, self.output_dim).to(self.device)
        print(self.state.size(0), self.drift_term.size(0))
    ...
    def train(self, save_path_logs):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min',         # minimize validation loss
                                                                factor=0.5,         # reduce LR by half
                                                                patience=100        # wait 100 epochs of no improvement
                                                            )
        loss_fn = nn.MSELoss()

        # Data
        x = self.state # input data
        y = self.drift_term # output data
        ...
        train_loader = DataLoader(train_dataset, batch_size = self.net_config.batch_size, shuffle=True)
        val_loader = DataLoader(val_dataset, batch_size = self.net_config.batch_size)
        ...
        torch.save(self.model, save_path_logs + f"drift_network.pth")
